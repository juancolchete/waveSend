...
contract MockSwapRouter {
    ...
    // Two-hop: CELO -> USDT -> WBTC (native value forwarded)
...
abstract contract WaveSendFundBase is Test {
    ...
    function setUp() public virtual {
        ...
        bytes memory initData = abi.encodeCall(
            ...
            (admin, address(usdtToken), address(wbtcToken), address(wsndToken), address(router), POOL_FEE, NATIVE_FEE, NATIVE_USDT_FEE)
        );
...
contract WaveSendFund_DepositNative is WaveSendFundBase {
    ...
    function test_receive_triggersDepositNative() public {
        // A plain CELO transfer to the contract calls depositNative(1).
        uint256 hashrateBefore = fund.totalPoolHashrate();
        vm.deal(address(this), 1 ether);
        (bool ok,) = address(fund).call{value: 1 ether}("");
        assertTrue(ok);
        // hashrate must have increased by WBTC_1 (router returns wbtcOutNative = WBTC_1)
